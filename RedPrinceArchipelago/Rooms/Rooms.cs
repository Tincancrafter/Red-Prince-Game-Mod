...
namespace RedPrinceArchipelago.Rooms
{
    public class ModRoomManager {
        ...
        public static List<string> CantCopy = ["ANTECHAMBER", "ENTRANCE HALL", "ROOM 46", "FOUNDATION", "THE FOUNDATION", ""];
            ...
            foreach (ModRoom room in Rooms)
            {
                ...
                if (room.IsUnlocked)
                {
                    ...
                    if (room.IsUnlocked && (room.RoomInHouseCount > 0 || room.RoomsLeftInPool > 0))
                    {
                        ...
                        SetPoolRemovalVar(room.GameObjectName, !dependenciesMet);
                        continue;
                    }
                }
        ...
        /// <summary>
        /// Keeps an Outer Room draft usable when none of the received Outer
        /// Rooms can currently be drafted.
        /// </summary>
        public void EnsureOuterRoomFallbacks()
        {
            ModRoom closet = GetRoomByName("CLOSET");
            if (closet?.GameObj == null) return;

            foreach (string pickerName in new[] { "STANDALONE ARRAY", "STANDALONE ARRAY FULL" })
            {
                if (!ModInstance.PickerDict.TryGetValue(pickerName, out PlayMakerArrayListProxy picker) || picker == null)
                {
                    continue;
                }
                if (picker.arrayList.Count > 0) continue;

                for (int i = 0; i < 3; i++)
                {
                    picker.Add(closet.GameObj, "GameObject");
                }
                Logging.Log($"No unlocked Outer Rooms were available in {pickerName}; added Closet fallbacks.", "Rooms");
            }
        }
